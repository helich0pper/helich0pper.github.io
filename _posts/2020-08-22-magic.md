...
```
Two ports to work with. OpenSSH (port 80) is up to date so we can disregard that for now. There also seems to be an Apache server running (port 80), the banner tells us it is an Ubuntu machine. Let's poke at that.
# Enumeration
<hr>
## Apache Server
...
So, if we go to that path and specify our uploaded image name and give the command we want to execute as a command it should return what we executed.
```
...
```
## theseus
Remember the SQL injection we did? There's bound to be some sort of credentials lying around since we can now read the back-end code.
```
...
```
Didn't work. Conveniently, the machine has mysqldump installed; maybe we can try to leak more credentials with the current credentials we have.
```
...
```
root@crab:~# ls; python3 -m http.server 80
